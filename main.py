...
import time
import os
from colorama import Fore  # Added this import statement
...
class NitroGen:
    ...
    def main(self):
        ...
        self.slowType(Fore.LIGHTMAGENTA_EX + "This Gen Is Working But Nitro Codes Are Too Rare To Get So It Will Take A Too While To Generate A Working Code", 0.02)

        self.slowType("                                                                       " , 0.04)
        self.slowType(Fore.BLUE + " Dev: THEGOODBALL", 0.02)
        self.slowType(Fore.YELLOW + " 💐 Discord ID - THEGOODBALL", 0.02)

        time.sleep(2)

        self.slowType("\nEnter amount of code to gen: ", 0.02, newLine=False)
...
Results:
 ...
 Valid Codes: {', '.join(valid)}""")
    ...
    def quickChecker(self, nitro, notify=None):
        ...
        response = requests.get(url)
        if response.status_code == 200:
            print(Fore.GREEN + f" WORKING | {nitro} ")  # Fixed color formatting
            if notify is not None:
                DiscordWebhook(
                    url=notify,
            ...
            print(Fore.RED + f" WRONG | {nitro} ")  # Fixed color formatting
            return False
